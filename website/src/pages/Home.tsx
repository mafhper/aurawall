...
import { Zap, Shield, Maximize, Play, Palette, Download, ArrowRight, Shuffle, Wand2, RefreshCw } from 'lucide-react';
import { PRESETS, DEFAULT_CONFIG } from '../../../src/constants';
...
export default function Home() {
  ...
  // Start with a random preset to make it interesting immediately
  const [heroConfig, setHeroConfig] = useState(() => {
    ...
    return {
      ...
      animation: {
        ...
        enabled: true, // Force animation enabled for hero
        speed: 2, // Slow and majestic
        flow: 3
      }
    };
  });
  ...
  const handleRandomize = () => {
    ...
    const availablePresets = PRESETS.filter(p => !p.config.baseColor || p.config !== heroConfig); // Simple inequality check
    ...
    setHeroConfig({
      ...
      animation: {
        ...randomPreset.config.animation,
        enabled: true,
        speed: Math.random() * 1.5 + 0.5, // Random speed 0.5-2.0 (Capped)
        flow: Math.random() * 1.5 + 0.5   // Random flow 0.5-2.0 (Capped)
      }
    });
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
             ...
             >
               ...
               <span className="relative z-10 w-24 text-left">
                  {/* Text slider/fader could be here, but simple text switch is fine */}
